...
def ask_for( key ):
    s = raw_input( "dict: enter value for '%s': " % key )
    try:
        val = eval(s)
    except NameError:
...
class pdict( dict ):
    ...
    def __getitem__( self, key ):
        if key not in self:
            if self.ask:
                print("pdict: parameter '%s' not found" % key)
                val = ask_for( key )
                print("pdict: setting '%s' = %s" % (key,repr(val)))
...
'''
dict = pdict.pdict()
dict.read_from_file(ParameterFile)

some_param = dict['some_params_dict_label']
'''
